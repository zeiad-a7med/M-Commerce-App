...
struct ProductsView: View {
    ...
    init(brandTitle: String) {
        ...
        _brandresponse = StateObject(wrappedValue: ProductsViewModel(first: 10, after: nil, filter: brandTitle))
        }
    var body: some View {
        
            VStack {
                FilterBar(filterItems: ["All Products",
                                        "Sale","Winter",
                                        "Spring","Autumn",
                                        "Summer"], willFilter: { filter in
                ...
                if brandresponse.isLoading {
                    ...
                    BrandsGrid(columns: columns
                               , products: brandresponse.products.BrandProducts ?? [], loadMore: {
                        brandresponse.loadMore(query: currentFilter)
                    })
                    .overlay(content: {
                        if brandresponse.products.BrandProducts?.count ?? 0 == 0 && !brandresponse.isLoading {
    ...
    func filterProducts() {
        ...
        if filterKeySelected != "" {
            self.brandresponse.fetchProducts(first: 20, after: nil, query: "\(brandTitle) | \(filterKeySelected)")
            currentFilter = "\(brandTitle) | \(filterKeySelected)"
        }else{
            self.brandresponse.fetchProducts(first: 20, after: nil, query: brandTitle)
