    ...
    var body: some View {
        NavigationView {
            VStack {
                        ...
                        if isMyOrdersSelected {
                            if let cust = orderViewModel.orderModel.customer {
                                ForEach(cust.orders?.nodes ?? [],id:\.id ) { ord in
                                    ...
                                    ,orderPrice: "\(ord.currentTotalPrice?.amount ?? "") \(ord.currentTotalPrice?.currencyCode ?? "")" ,date: ord.processedAt ?? ""
                                    ,noOfProducts: 2,
                                    isMyOrder: isMyOrdersSelected,
                                    detailsDest: AnyView(DetailsView(numberOfOrder: 0)),
                ...
                if orderViewModel.orderModel.customer?.orders?.nodes?.count ?? 0 == 0 && !orderViewModel.isLoading && isMyOrdersSelected {
                    let msg = "No Orders Found"
                    let desc = Text("You don't have any orders yet. Place your first order now and start shopping!")
