...
//    @State var subFilter: String?
    var body: some View {
        LazyVGrid(columns: columns, spacing: 10) {
            ForEach(modelView.filteredProducts.products, id: \.id) { product in
                ProductCardView(product: Product(id: product.id, title: product.title , vendor: product.vendor,priceRange: product.priceRange,
            ...
            modelView.searchOnProduct(filterProduct: "\(mainFilter ?? "") | \(searchText)")
        })
    }
}
