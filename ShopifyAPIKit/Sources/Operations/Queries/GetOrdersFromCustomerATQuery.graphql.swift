...
public class GetOrdersFromCustomerATQuery: GraphQLQuery {
  ...
  public static let operationDocument: ApolloAPI.OperationDocument = .init(
    definition: .init(
      #"query GetOrdersFromCustomerAT($CustomerAT: String!) { customer(customerAccessToken: $CustomerAT) { __typename id phone email orders(first: 100, reverse: true) { __typename totalCount nodes { __typename currencyCode edited email financialStatus fulfillmentStatus id name orderNumber processedAt statusUrl currentSubtotalPrice { __typename amount currencyCode } currentTotalPrice { __typename amount currencyCode } } } addresses(first: 1) { __typename nodes { __typename address1 city country countryCode firstName id lastName latitude longitude name phone } } displayName } }"#
    ))
